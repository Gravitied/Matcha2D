...
export class World {
  ...
  static async create(config: Partial<WorldConfig> = {}): Promise<World> {
    ...
    // Rust solver: num_iterations = outer substeps;
    // num_internal_iterations = PGS passes per substep.
   ...
   */
  setWorldBounds(bounds: { minX: number; minY: number; maxX: number; maxY: number } | null): void {
    if (bounds) {
  ...
  resolveOverlapsForBody(handle: BodyHandle): void {
    this._engine.resolve_overlaps_for_body(handle as unknown as number)
  }
