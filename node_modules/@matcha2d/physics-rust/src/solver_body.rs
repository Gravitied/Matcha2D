...
/// How a body’s pose is advanced each solver substep (`integrate_positions`).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PoseIntegration {
    /// Static: pose fixed for the step.
    Static,
    /// Dynamic: full solver integration (forces, impulses, slippage).
    Dynamic,
    /// Kinematic: integrate from linear/angular velocity only (`inv_mass` / `inv_inertia` stay 0 for constraints).
    Kinematic,
}
...
pub struct SolverPose {
    ...
    pub inv_inertia: f32,
    pub inv_mass: f32,
    pub integration: PoseIntegration,
}
...
impl Default for SolverPose {
    fn default() -> Self {
        Self {
            ...
            inv_inertia: 0.0,
            inv_mass: 0.0,
            integration: PoseIntegration::Static,
        }
    }
}
...
pub struct SolverBodySet {
    ...
    pub capacity: usize,
    /// Hard cap on |linear velocity| before position integration each substep (all dynamic/kinematic bodies).
    pub max_integrate_linear_velocity: f32,
    /// Hard cap on |angular velocity| before position integration each substep.
    pub max_integrate_angular_velocity: f32,
}
...
impl SolverBodySet {
    pub fn new() -> Self {
        Self {
            ...
            capacity: 0,
            // No cap by default — discrete TOI / substeps handle fast motion; arbitrary limits hide solver error.
    ...
    pub fn copy_from_bodies(&mut self, bodies: &RigidBodySet, dt: f32, gravity: Vec2) {
        ...
        for &body_idx in &indices {
            ...
            // Inverse mass for constraints; kinematic keeps 0 mass but still integrates from velocity.
            if body.is_dynamic() {
                self.poses[body_idx].inv_mass = body.inv_mass;
                self.poses[body_idx].inv_inertia = body.inv_inertia;
                self.poses[body_idx].integration = PoseIntegration::Dynamic;
            } else if body.is_kinematic() {
                self.poses[body_idx].inv_mass = 0.0;
                self.poses[body_idx].inv_inertia = 0.0;
                self.poses[body_idx].integration = PoseIntegration::Kinematic;
            } else {
                self.poses[body_idx].inv_mass = 0.0;
                self.poses[body_idx].inv_inertia = 0.0;
                self.poses[body_idx].integration = PoseIntegration::Static;
            }
    ...
    pub fn apply_damping(&mut self, dt: f32) {
        for i in 0..self.capacity {
            if self.poses[i].integration == PoseIntegration::Kinematic {
                continue;
            }
    ...
    pub fn integrate_positions(&mut self, dt: f32) {
        for i in 0..self.capacity {
            ...
            let integrate = pose.integration == PoseIntegration::Dynamic
                || pose.integration == PoseIntegration::Kinematic;
            if !integrate {
                continue;
            }
            ...
            let max_lin = self.max_integrate_linear_velocity;
            if max_lin.is_finite() && max_lin > 0.0 {
                let len = vel.linear.length();
                if len > max_lin {
                    vel.linear = vel.linear * (max_lin / len.max(1e-20));
                }
            }
            let max_ang = self.max_integrate_angular_velocity;
            if max_ang.is_finite() && max_ang > 0.0 && vel.angular.abs() > max_ang {
                vel.angular = vel.angular.signum() * max_ang;
            }

            pose.translation += vel.linear * dt;
            let angle = pose.rotation.angle() + vel.angular * dt;
    ...
    pub fn writeback_to_bodies(&self, bodies: &mut RigidBodySet) {
        ...
        for &body_idx in &indices {
            ...
            // Clear accumulated forces for all dynamic bodies each step.
            // Forces are re-applied by the user each frame (article: forces zeroed after integration).
            if body.is_dynamic() {
