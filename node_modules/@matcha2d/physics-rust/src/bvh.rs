...
impl DynamicBvh {
    ...
    /// Insert a leaf. `margin` expands the stored AABB (`AABB_MARGIN` for the persistent dynamic tree).
    /// Use `margin = 0.0` for ephemeral trees that must match exact AABB overlap (e.g. swept broadphase).
    pub fn insert_leaf_with_margin(
        &mut self,
        aabb: AABB,
        ...
        self.nodes[leaf].aabb = if margin > 0.0 {
            expand_aabb(aabb, margin)
        } else {
            aabb
        };
    ...
    pub fn insert_leaf(&mut self, aabb: AABB, handle: ColliderHandle) -> usize {
        self.insert_leaf_with_margin(aabb, handle, AABB_MARGIN)
    }
...
mod tests {
    ...
    /// `query_pairs` with zero insert margin must match brute-force on raw AABBs (swept broadphase semantics).
    #[test]
    fn test_query_pairs_matches_bruteforce_many_boxes_exact() {
        let mut aabbs: Vec<(ColliderHandle, AABB)> = Vec::new();
        let mut h = 0u32;
        for gx in 0..8 {
            for gy in 0..8 {
                let x0 = gx as f32 * 1.1;
                let y0 = gy as f32 * 1.1;
                aabbs.push((
                    ColliderHandle(h),
                    AABB::new_from_coords(x0, y0, x0 + 1.0, y0 + 1.0),
                ));
                h += 1;
            }
        }
        let mut bvh = DynamicBvh::new();
        for (ch, a) in &aabbs {
            bvh.insert_leaf_with_margin(*a, *ch, 0.0);
        }
        let mut qp = bvh.query_pairs();
        qp.sort_by_key(|(a, b)| (a.0, b.0));

        let n = aabbs.len();
        let mut brute: Vec<(ColliderHandle, ColliderHandle)> = Vec::new();
        for i in 0..n {
            for j in (i + 1)..n {
                if aabbs[i].1.overlaps(aabbs[j].1) {
                    let (ca, cb) = (aabbs[i].0, aabbs[j].0);
                    brute.push(if ca.0 < cb.0 { (ca, cb) } else { (cb, ca) });
                }
            }
        }
        brute.sort_by_key(|(a, b)| (a.0, b.0));
        assert_eq!(qp, brute, "BVH pair set must equal brute-force on exact AABBs");
    }
}
