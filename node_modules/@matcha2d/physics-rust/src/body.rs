...
impl RigidBodySet {
    ...
    /// Total arena slots (including freed indices).
    ...
    /// `true` if slot `idx` is an active (non-freed) body.
    pub fn slot_is_live(&self, idx: usize) -> bool {
        idx < self.bodies.len() && !self.free_indices.contains(&idx)
    }

    /// Full generational handle for a body at a slot index (including freed slots — caller must filter).
    pub fn handle_for_index(&self, idx: usize) -> RigidBodyHandle {
        RigidBodyHandle(idx as u32 | ((self.generations[idx] as u32) << 24))
    }
}
